...
function draw() {
  background(0); //背景為黑色
// translate(width/2,height/2) //把原點由視窗的左上角移到視窗的中心
for(let y =0;y<height;y =y+120){
for(let x =0;x<width;x =x+120){
  push() //重新設定格式(包含原點位置，線條顏色)，要設定原點(0,0)的位置在(x,y)上
    translate(x,y) //把原點(0,0)放在滑鼠游標上
  stroke(255) //線條顏色
for(let i=0;i<10;i = i+1){
    let r = map (sin(frameCount),-1,-1,50,255)
    let g = map (cos(frameCount/2),-1,1,50,70)
    let b = map (cos(frameCount/4),-1,1,50,255)
    stroke(r,g,b)
